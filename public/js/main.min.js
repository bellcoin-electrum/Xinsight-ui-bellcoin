...
function UnparsedTXOTryParse(scriptPubKey,u){
   var res = "Unparsed address ["+ u +"]";
   var m;
   var len;
   if (scriptPubKey["hex"]){
      res = "Non-standard TXO <" + scriptPubKey["hex"] + "> [" + u + "]";
      m = /^6a([0-9a-f]{2})(.*)/.exec(scriptPubKey["hex"]);
      if (m){
        len = parseInt(m[1],16);
        if (len > 0){
          if (len <= 75){
	      res = "Data <" + utf8_hex_string_to_string(scriptPubKey["hex"].substring(4)) + ">";
          }else{
             switch(len){
               case 76:
                   //76-255 bytes data
                   res = "Large Data <" + utf8_hex_string_to_string(scriptPubKey["hex"].substring(6)) + ">";
                   break;
               case 77:
                   //256-65535 bytes data
                   res = "ExLarge Data <" + utf8_hex_string_to_string(scriptPubKey["hex"].substring(8)).substring(0,255) + "...>";
                   break;
               case 78:
                   //up to 4294967295 bytes data...!?
                   res = "ExExLarge Data<" + utf8_hex_string_to_string(scriptPubKey["hex"].substr(12,512)) + "...>";
		   break;
               default:
                   break;
             }
